...
const searchData = [
  { title: 'Home', description: 'Go to homepage', href: '/' },
  { title: 'About Us', description: 'Learn more about our company', href: '/about' },
  { title: 'Services', description: 'Explore our housekeeping, security, manpower and related services', href: '/services' },
  { title: 'Projects', description: 'View our completed and ongoing projects', href: '/projects' },
  { title: 'Clients', description: 'See our clients and partnerships', href: '/clients' },
  ...
  { title: 'Terms & Conditions', description: 'Read our terms and conditions', href: '/terms' },
];

// Map original nav item keys to display text
const displayNameMap = {
  About: 'About Us',
  Contact: 'Contact Us',
  // All others keep the same name (Home, Services, Projects, Clients)
};
...
export default function Header() {
  ...
  // Helper to get the display text for a nav item
  const getDisplayName = (item) => displayNameMap[item] || item;

  // Helper to get href (unchanged logic)
  const getHref = (item) =>
      ...
      <div className="flex w-full items-center justify-between gap-4 px-4 py-3 md:px-6 lg:px-12">
        <div className="flex items-center gap-3">
          <div className="flex h-[71px] w-[65px] items-center justify-center">
            <img src="/images/logo.jpg" alt="logo" className="h-full w-full object-contain" />
          </div>
        </div>
          ...
          <div className="relative hidden md:block">
            ...
            >
              <input
                ...
                placeholder="Search"
              />
              <button type="submit" className="text-[32px] text-[#6b4d33] leading-none">
                ⌕
              </button>
            </form>
              ...
              <div className="absolute left-0 top-[56px] z-50 w-full overflow-hidden rounded-[20px] border border-[#e4d2bf] bg-white shadow-lg">
                {results.length > 0 ? (
                  results.slice(0, 6).map((item) => (
                    ...
                    >
                      <p className="text-[16px] font-medium text-[#53362D]">{item.title}</p>
                      <p className="text-[13px] text-[#7b746d]">{item.description}</p>
                    </Link>
                  ))
                ) : (
                  <div className="px-4 py-3 text-[14px] text-[#7b746d]">No results found</div>
                )}
              </div>
            )}
          </div>
                ...
                <li key={item} className="h-full">
                  <Link
                    ...
                    className={`flex h-full items-center justify-center px-8 text-[20px] font-normal text-white transition-colors ${
                      isActive ? 'bg-[#9C6A2F]' : 'bg-transparent hover:bg-[#B97E3F]'
                    }`}
                  >
                    {displayName}
                  </Link>
                ...
                return (
                  <li
                    ...
                    className={`w-full py-3 text-left text-[20px] font-medium ${
                      isActive ? 'text-[#53362D] border-l-4 border-[#53362D] pl-2' : 'text-white'
                    }`}
                  >
